...
```bash
.venv/bin/python3 experiments/lena_p4_run/scripts/prepare_lena_gold.py
```
...
```text
experiments/lena_p4_run/articles_4_lena/
```
...
```bash
.venv/bin/python3 experiments/lena_p4_run/scripts/run_lena_p4_answers.py --check-inputs-only
```

## Run P4 answers only

```bash
.venv/bin/python3 experiments/lena_p4_run/scripts/run_lena_p4_answers.py
```
...
```bash
.venv/bin/python3 experiments/lena_p4_run/scripts/run_lena_p4_answers.py --resume-output experiments/lena_p4_run/outputs_lena_p4/<partial-output>.jsonl
```

The output will be named like:

```text
experiments/lena_p4_run/outputs_lena_p4/lena_run4_p4_answers_<timestamp>_papers33_p01-33_q1-49_nq1617.jsonl
```

## View Outputs In The Web UI

The web UI auto-loads the bundled JSONL file in `experiments/lena_p4_run/webui/data/`, so the `webui/` folder can be uploaded as a static site. When served from this repository, it can also refresh and read JSONL files from `experiments/lena_p4_run/outputs_lena_p4/`.
...
```text
http://127.0.0.1:8000/experiments/lena_p4_run/webui/
```
