...
class P2_Imp_Pipeline(BaseGraphPipeline):
    def __init__(self, pipeline_version: str, config: Any, openai_client: Any, logger: Any, run_id: str):
        ...
        self._bm25_cache: Dict[int, Dict[str, Any]] = {}
    ...
    def _sparse_candidates(self, query: str, n_results: int, paper_id: int) -> List[Dict[str, Any]]:
        if paper_id not in self._bm25_cache:
        ...
        cache_row = self._bm25_cache.get(paper_id, {})
        bm25 = cache_row.get("bm25")
        ids = cache_row.get("ids", []) or []
        docs = cache_row.get("docs", []) or []
        metadatas = cache_row.get("metadatas", []) or []
