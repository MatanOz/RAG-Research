...
const el = {
  ...
  mPerQuestionCost: document.getElementById("mPerQuestionCost"),
  mPaperBuildTokens: document.getElementById("mPaperBuildTokens"),
  mPaperBuildCost: document.getElementById("mPaperBuildCost"),
  mRunCost: document.getElementById("mRunCost"),
...
function renderMetrics() {
  ...
  const avgLatency = data.length
    ...
    : null;
  const perQuestionTotalCost = data.reduce((acc, r) => acc + (r.logs?.estimated_cost_usd || 0), 0);
  const perQuestionAvgCost = data.length ? perQuestionTotalCost / data.length : 0;
  const avgTop1 =
    data.length > 0
  ...
  const paperBuildTokensMap = new Map();
  const paperBuildCostMap = new Map();
  data.forEach((r) => {
    const pid = r.paper_id;
    const tokens = r.logs?.paper_build_embedding_tokens || 0;
    const cost = r.logs?.paper_build_estimated_cost_usd || 0;
    const existingTokens = paperBuildTokensMap.get(pid) || 0;
    const existingCost = paperBuildCostMap.get(pid) || 0;
    if (tokens > existingTokens) paperBuildTokensMap.set(pid, tokens);
    if (cost > existingCost) paperBuildCostMap.set(pid, cost);
  });

  const totalPaperBuildTokens = [...paperBuildTokensMap.values()].reduce((a, b) => a + b, 0);
  const totalPaperBuildCost = [...paperBuildCostMap.values()].reduce((a, b) => a + b, 0);
  const totalRunCost = perQuestionTotalCost + totalPaperBuildCost;
  ...
  el.mCost.textContent = `$${formatNum(perQuestionTotalCost, 6)}`;
  el.mPerQuestionCost.textContent = `$${formatNum(perQuestionAvgCost, 6)}`;
  el.mPaperBuildTokens.textContent = totalPaperBuildTokens.toLocaleString();
  el.mPaperBuildCost.textContent = `$${formatNum(totalPaperBuildCost, 6)}`;
  el.mRunCost.textContent = `$${formatNum(totalRunCost, 6)}`;
